...
window.onload = function () {
	console.log("bg.getUserData in window.onload ");
	...
	save.onclick = function login() {
		console.log("save button clicked user == " + username.value);
		console.log("save button clicked user == " + username.value.length);
		...
		if (username.value.length <= 0 )  {
			...
			console.log("Do the UserData Update");
			settings.addPromptedUsername(username.value, function(json, event) {
				console.log('userPrompt:  ' + JSON.stringify(json, null, 2))
				console.log('userPrompt:  <' + username.value + '>');
				const userhandler = require('../js/handleUserData');
				connect.checkConnectivity(json, function(obj) {
					console.log('userPrompt:  ' + JSON.stringify(obj, null, 2))
						if (obj.code == 200) { 					
							ipcRenderer.send('complete-user-data', pjson.config.mainurl, function() {
								console.log('userPrompt:  Why are we here? ' + JSON.stringify(obj, null, 2))
